# Run through Codex full-course to identify features absent from our current Claude Code workflow

> Back to [[experiments-index]]

Source: **[Master 97% of Codex in 1 Hour (full course)](https://www.youtube.com/watch?v=3TdD8Qv5Tk8)** · NateHerk · 2026-05-09

**Status:** `discarded` · **Verdict:** `discard` · **Effort:** `low`

---

## Hypothesis

If we systematically review Codex capabilities against our current Claude Code usage, then we will find underutilized features or patterns that can be ported or adapted to improve our dev loop because a structured course surfaces the full feature surface rather than only what we discover organically.

## What they did

Inferred from title: speaker produced a comprehensive one-hour course covering the majority of OpenAI Codex's features and use patterns.

## Relevance to YOLO loop

Useful as a comparative reference; watching with a gap-analysis mindset against our current loop could surface quick wins.

## Notes

Discarded 2026-05-10: tutorial/course recap; not an experiment.

## Status history

| Date | Status | Note |
|---|---|---|
| 2026-05-09 | `backlog` | Extracted from YouTube RSS |
|  | `` |  |

---

## Metadata

| Field | Value |
|---|---|
| Experiment ID | `nh-2026-05-09-codex-full-course` |
| Channel | NateHerk |
| Video | [Master 97% of Codex in 1 Hour (full course)](https://www.youtube.com/watch?v=3TdD8Qv5Tk8) |
| Published | 2026-05-09 |
| Ingested upstream | 2026-05-09 |
| Source | [yolo-projects/experiments.json](https://github.com/Anguijm/yolo-projects/blob/main/experiments.json) |

---

## Related pages

- [[yolo-projects]] - upstream pipeline that synthesized this experiment
- [[yolo-phase4-integration]] - how experiments are synced into this wiki
- [[experiments-index]] - all experiments
- [[index]] - wiki home
